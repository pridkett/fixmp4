...
def md5(fname: str, desc: str=None) -> str:
    ...
    with tqdm(total=os.path.getsize(fname), unit='B', unit_scale=True,
...
def process_dir(dirname: str) -> None:
    ...
    if 'md5' not in json_metadata.get('original', {}):
        md5sum = md5(os.path.join(dirname, video),
                     desc=f'original md5 {video}')
    ...
    def transcode():
        ...
        outcommand = ['ffmpeg', '-v', 'warning', '-i', os.path.join(dirname, video), '-c', 'copy', '-map', '0', os.path.join(dirname, outfilename)]
    ...
    with tqdm(total=os.path.getsize(os.path.join(dirname, video)), unit='B',
              unit_scale=True, miniters=1, desc=f'ffmpeg {video}',
              leave=False) as pbar:
        old_filesize = 0
        while True:
    ...
    md5sum = md5(os.path.join(dirname, outfilename),
                 desc=f'converted md5 {video}')
...
def main() -> None:
    ...
    dirs = sorted(dirs)
    for this_dir in tqdm(dirs,
                         desc='{BOLD}{RED}Total Progress{END}'.format(**ansi)):
